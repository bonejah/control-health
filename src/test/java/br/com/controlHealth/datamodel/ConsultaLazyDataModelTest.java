...
public class ConsultaLazyDataModelTest {
	...
	public void shouldLoadDataModel() {
		...
		List<Consulta> load = model.load(0, 0, "", SortOrder.ASCENDING, new HashMap<String, Object>());
	...
	public void shouldGetRowKey() {
		Mockito.when(model.getRowKey(Mockito.any(Consulta.class))).thenReturn(new Consulta());
		
		Object object = model.getRowKey(new Consulta());
	...
	public void shouldGetRowData() {
		Mockito.when(model.getRowData()).thenReturn(new Consulta());
		
		Consulta consulta = model.getRowData();

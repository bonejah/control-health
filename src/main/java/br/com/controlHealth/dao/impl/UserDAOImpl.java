...
public class UserDAOImpl implements DAO<User> {
	...
	private static final long serialVersionUID = 3394054109502827357L;
	...
	public List<User> getAll() {
		...
		try {
			CriteriaQuery<User> query = this.em.getCriteriaBuilder().createQuery(User.class);
	...
	@Override
	public List<User> getListPaginate(int start, int size, String sortField, boolean asc,
		...
		try {
			...
			TypedQuery<User> query = this.em.createQuery(select);
	...
	public User checkIfExistsUser(User user) {
		User userFound = null;
		TypedQuery<User> query = em.createQuery("select user from User user where user.email = :pEmail", User.class);
		...
		try {
			...
			System.err.println("UsuarioDAO - Error method checkIfExistsUser: " + e.getMessage());
		}
